...
// const even = mixedNumbers.filter(arr => arr % 2 === 0)
// console.log(even)
...
// const postTaxPrice = prices.map(elm => elm * 1.07) 
...
// const totalPopulation = populations.reduce((acc, elm) => acc + elm)
...
// const myStrongest = monstersInYourPocket.filter(monster => monster.CP > 200).map(elm => elm)
...
// const totalPrice = orders.map((elm,) => elm.price * (1 + elm.tax))
...
// CODE HERE
const bobsTotal = purchases.filter(elm => elm.owner === `Bob`).reduce((acc, elm) => acc + elm.price, 0)
console.log(bobsTotal)
// why do we need a zero